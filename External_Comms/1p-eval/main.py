...
from eval_1p_thread import start_all_threads
...
def main():
    # ask the user if the evaluation server is present
    # yes will set true
    eval_server_present = input("Is the evaluation server present? (y/n): ").strip().lower() == "y"

    # uncomment for NO eval_server
    # eval_client = None

    # uncomment for eval_server
    # eval_server_present = True
    ...
    start_all_threads(ai, AI_data_queue, visualiser_queue_1, visualiser_queue_2, mqtt_client, players, running_event, node_relay_server, eval_server_queue, eval_client, hardware_data_queue, unity_stats_queue)
