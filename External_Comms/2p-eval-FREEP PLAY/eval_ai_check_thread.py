        ...
        try:
            ...
            if packet_count <= 60:
                ...
                if device_id == 1 and not device_1_inferred:
                    ...
                    ai_action_output = json.loads(ai.inference(device_data_list, "glove"))
                    # player_id = 1
                    ...
                    print(f"{ConsoleColors.DEVICE4}Device {device_id} length = {len(device_data_list)}{ConsoleColors.RESET}")
                    ai_action_output = json.loads(ai.inference(device_data_list, "soccer"))
                    action = str(ai_action_output.get("action"))        # just added
                    ai_action_output["type"] = "action"
                    visualiser_queue_1.put(json.dumps(ai_action_output))
                    print(f"{ConsoleColors.AIOUTPUT}{action} put into visualiser_queue_1{ConsoleColors.RESET}")
                    device_4_inferred = True
                    ...
                    print(f"{ConsoleColors.DEVICE8}Device {device_id} length = {len(device_data_list)}{ConsoleColors.RESET}")
                    ai_action_output = json.loads(ai.inference(device_data_list, "soccer"))
                    action = str(ai_action_output.get("action"))
                    ai_action_output["type"] = "action"
                    visualiser_queue_2.put(json.dumps(ai_action_output))
                    print(f"{ConsoleColors.AIOUTPUT}{action} put into visualiser_queue_2{ConsoleColors.RESET}")
                    device_8_inferred = True
...
def start_all_threads(ai, AI_data_queue, visualiser_queue_1, visualiser_queue_2, mqtt_client, players, running_event, node_relay_server, eval_server_queue, eval_client, hardware_data_queue, unity_stats_queue, p1_response_flag, p2_response_flag):
    ...
    # testing thread
    threading.Thread(target=test_action_from_user_thread, args=(visualiser_queue_1, visualiser_queue_2), daemon=True).start()
...
# ======= AI Test Threads =========
def AI_test_thread(ai, AI_data_queue, running_event):
    threshold = 60
    device_data_map = {
        1: [],  # p1_glove
        4: [],  # p1_leg
    ...
    while running_event.is_set():
        try:
            ...
            # Append data only if packet_count <= threshold
...
# # ========== version 1 working with progress bar in console ==========
# def AI_test_thread(ai, AI_data_queue, running_event):
#     device_data_map = {
#         1: [],  # p1_glove
#         4: [],  # p1_leg
...
#     # Initialize counts and inference flags for each device
#     device_count_map = {1: 100, 4: 100, 5: 100, 8: 100}
#     device_inferred_map = {1: False, 4: False, 5: False, 8: False}
#     threshold = 60  # Threshold for AI inference
...
#             # Reset the data list if packet count decreases
#             if packet_count < device_count_map[device_id]:
#                 device_data_list.clear()
#                 device_inferred_map[device_id] = False
#             device_count_map[device_id] = packet_count

#             # Update and print progress bar
#             progress = min(len(device_data_list), threshold)
#             print_progress_bar(device_id, progress, threshold)

#             # Collect data if packet_count <= 60
#             if packet_count <= threshold:
#                 device_data_list.append(data)
#             else:
#                 # Perform AI inference only once for packet_count > 60
#                 if device_id == 1 and not device_inferred_map[device_id]:
#                     ai_action_output = json.loads(ai.inference(device_data_list, "glove"))
#                     print(f"\n{ConsoleColors.DEVICE1}P1 Action from Device {device_id}: {ai_action_output['action']}{ConsoleColors.RESET}")
#                     # clear_progress_bar()
#                     device_inferred_map[device_id] = True

#                 elif device_id == 4 and not device_inferred_map[device_id]:
#                     ai_action_output = json.loads(ai.inference(device_data_list, "soccer"))
#                     print(f"\n{ConsoleColors.DEVICE4}P1 Action from Device {device_id}: {ai_action_output['action']}{ConsoleColors.RESET}")
#                     # clear_progress_bar()
#                     device_inferred_map[device_id] = True

#                 elif device_id == 5 and not device_inferred_map[device_id]:
#                     ai_action_output = json.loads(ai.inference(device_data_list, "glove"))
#                     print(f"\n{ConsoleColors.DEVICE5}P2 Action from Device {device_id}: {ai_action_output['action']}{ConsoleColors.RESET}")
#                     # clear_progress_bar()
#                     device_inferred_map[device_id] = True

#                 elif device_id == 8 and not device_inferred_map[device_id]:
#                     ai_action_output = json.loads(ai.inference(device_data_list, "soccer"))
#                     print(f"\n{ConsoleColors.DEVICE8}P2 Action from Device {device_id}: {ai_action_output['action']}{ConsoleColors.RESET}")
#                     # clear_progress_bar()
#                     device_inferred_map[device_id] = True
...
# def update_progress_via_socketio(device_id, progress, threshold, action=None):
#     """Emit a real-time progress update event with optional action via WebSocket."""
#     progress_percent = min(int((progress / threshold) * 100), 100)
#     # Send progress and action to the front end
#     sio.emit('progress_update', {'device_id': device_id, 'progress': progress_percent, 'action': action})
...
def update_progress_via_socketio(device_id, progress, threshold, action=None):
    ...
    data = {'device_id': device_id, 'progress': progress_percent}
    if action is not None:
        data['action'] = action  # Only add 'action' if it's provided

    # Emit data to the frontend
    sio.emit('progress_update', data)



def AI_FE_test_thread(ai, AI_data_queue, running_event):
    device_data_map = {
        1: [],  # p1_glove
        4: [],  # p1_leg
        5: [],  # p2_glove
    ...
    while running_event.is_set():
        try:
            ...
            progress = min(len(device_data_list), threshold)
            update_progress_via_socketio(device_id, progress, threshold)  # Emit progress to the web app

            # Collect data if packet_count <= threshold
            if packet_count <= threshold:
                device_data_list.append(data)
            else:
                # Perform AI inference only once for packet_count > threshold
                if device_id == 1 and not device_inferred_map[device_id]:
                    ai_action_output = json.loads(ai.inference(device_data_list, "glove"))["action"]
                    print(f"\nP1 Action from Device {device_id}: {ai_action_output}")
                    update_progress_via_socketio(device_id, 0, threshold, ai_action_output)  # Send progress with action
                    device_inferred_map[device_id] = True

                elif device_id == 4 and not device_inferred_map[device_id]:
                    ai_action_output = json.loads(ai.inference(device_data_list, "soccer"))["action"]
                    print(f"\nP1 Action from Device {device_id}: {ai_action_output}")
                    update_progress_via_socketio(device_id, 0, threshold, ai_action_output)  # Send progress with action
                    device_inferred_map[device_id] = True
                    ...
                    ai_action_output = json.loads(ai.inference(device_data_list, "glove"))["action"]
                    print(f"\nP2 Action from Device {device_id}: {ai_action_output}")
                    update_progress_via_socketio(device_id, 0, threshold, ai_action_output)  # Send progress with action
                    device_inferred_map[device_id] = True
...
# Ensure to disconnect from the Socket.IO server when done
@sio.event
def disconnect():
...
def generate_random_IMU_data(player_id, device_id, count):
    """Generate a row of IMU data with the given player_id, device_id, and count."""
    data = {
        "player_id": player_id,
        "device_id": device_id,
        "accX": random.uniform(-10.0, 10.0),
        "accY": random.uniform(-10.0, 10.0),
        "accZ": random.uniform(-10.0, 10.0),
        "gyrX": random.uniform(-500.0, 500.0),
        "gyrY": random.uniform(-500.0, 500.0),
        "gyrZ": random.uniform(-500.0, 500.0),
        "count": count
    }
    return data

def populate_AI_thread(AI_data_queue):
    while True:
        try:
            # Input player_id and device_id from user
            player_id = int(input("Input player_id (1 or 2): "))
            device_id = int(input("Input device_id (1, 4, 5, or 8): "))
            
            # Generate and enqueue 65 rows of IMU data
            for count in range(1, 66):
                imu_data = generate_random_IMU_data(player_id, device_id, count)
                AI_data_queue.put(json.dumps(imu_data))
                # print(f"Data for count {count} put into AI_data_queue")
                time.sleep(0.1)  # Optional: delay between enqueues for better visualization
...
def start_AI_test_threads(ai, AI_data_queue, running_event, node_relay_server, hardware_data_queue, visualiser_queue_1, visualiser_queue_2, eval_client, players, unity_stats_queue):
    threading.Thread(target=node_relay_server.start, args=(AI_data_queue, hardware_data_queue, visualiser_queue_1, visualiser_queue_2), daemon=True).start()
    # threading.Thread(target=AI_test_thread, args=(ai, AI_data_queue, running_event), daemon=True).start()
    threading.Thread(target=AI_FE_test_thread, args=(ai, AI_data_queue, running_event), daemon=True).start()
    threading.Thread(target=populate_AI_thread, args=(AI_data_queue,), daemon=True).start()
