...
struct SoccerPacket {
  byte typeOfPacket = SOCCER_PACKET;
  byte seqNum;
  byte padding[17] = {0};
  byte checkSum;
};
...
void sendSoccerPkt() {
  SoccerPacket soccerPkt;
  soccerPkt.seqNum = seq_Num;
  crc.reset();
  crc.add((byte*)&soccerPkt, SIZE_OF_PACKET - sizeof(soccerPkt.checkSum));
  soccerPkt.checkSum = crc.getCRC();
  memcpy(sendBuffer, &soccerPkt, sizeof(soccerPkt));
