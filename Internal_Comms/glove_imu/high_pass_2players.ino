...
float accX = 0, accY = 0, accZ = 0;  // For accelerometer values
float grav_accX = 0, grav_accY = 0, grav_accZ = 0, grav_ema_acc = 0;// With gravity
float gyroX = 0, gyroY = 0, gyroZ = 0;  // For gyroscope values
sensors_event_t a, g, temp;
int16_t dataArray[80];
int arrayIndex = 0;
const int buzzerPin = A0; // Change to any available digital pin
...
void getIMUData(int16_t accX, int16_t accY, int16_t accZ, int16_t gyrX, int16_t gryY, int16_t gryZ, int16_t ema_acc, int16_t ema_gyr) {
  DataPacket dataPkt;
...
void startOfAction() {
  NewTone(buzzerPin, 2100, 150); // Play 2000 Hz for 200ms
  delay(150);
  NewTone(buzzerPin, 2500, 150); // Play 2500 Hz for 200ms
  delay(150);
  noNewTone(buzzerPin);          // Stop the tone
}

void endOfAction() {
  NewTone(buzzerPin, 2500, 150); // Play 2000 Hz for 200ms
  delay(150);
  NewTone(buzzerPin, 2100, 150); // Play 2500 Hz for 200ms
  delay(150);
  noNewTone(buzzerPin);          // Stop the tone
}
...
void loop() {
  ...
  while (handshakeDone) {
    ...
    grav_accY = a.acceleration.y * 100;
    grav_accZ = a.acceleration.z * 100;
    float grav_mag_acc = abs(sqrt(pow(grav_accX, 2) + pow(grav_accY, 2) + pow(grav_accZ, 2)) - 1000);
    grav_ema_acc = smoothing * grav_ema_acc + (1 - smoothing) * grav_mag_acc;
    // Store gyroscope values in float variables
    gyroX = g.gyro.x * 100;
    gyroY = g.gyro.y * 100;
    gyroZ = g.gyro.z * 100;
    ...
    //unsigned long elapsedTime = millis() - startTime;11111111111
    char incomingResponse = Serial.read();
    if (incomingResponse == HELLO_PACKET) {
      handshakeDone = false;
      sendAckPkt();
    ...
    if ((ema_acc > 700 && (abs(accY) + abs(accX)) > 1400) || (gyroY > 600)) {
      isAction = true;
    }
    if (isAction) {
      count += 1;
      ...
      if (count == 65) {
        getIMUData(int16_t(dataArray[arrayIndex]), int16_t(dataArray[arrayIndex + 1]), int16_t(dataArray[arrayIndex + 2]), int16_t(dataArray[arrayIndex + 3]), int16_t(dataArray[arrayIndex+ 4]), int16_t(dataArray[arrayIndex + 5]), int16_t(dataArray[arrayIndex + 6]), int16_t(dataArray[arrayIndex + 7]));
  ...
  dataArray[arrayIndex + 6] = grav_ema_acc;
  dataArray[arrayIndex + 7] = ema_gyr;

  arrayIndex += 8;
  arrayIndex = (arrayIndex >= 80) ? 0 : arrayIndex; 
  delay(25);
  }
}